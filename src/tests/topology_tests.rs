...
pub mod topology_tests {
    ...
    fn should_find_best_path() {
        ...
        for node in route.clone() {
            ...
            for i in 0..10 {
                ...
                topology.update_node_history(
                    &vec![node],
                    rand::thread_rng().gen_range(0.0..1.0) < PDR
                );
            }
        }
        ...
        assert_eq!(route.get(route.len() - 1), new_route.get(new_route.len() - 1));
    ...
    pub fn update_topology(topology: &mut Topology, nodes: Vec<NodeId>, edges: Vec<(NodeId, NodeId)>) {

        // Ad node to node list if not there
        for node in nodes {
            if !topology.nodes().contains(&node) { topology.add_node(node); }
        }
